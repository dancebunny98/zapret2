...
Для установки и запуска на pfSense используйте отдельную инструкцию : [Настройка pfSense](pfsense.md).

## Перехват трафика в ядре OpenBSD

В OpenBSD есть только pf. С механизмом предотвращения зацикливания divert все в порядке.

Практическая конфигурация двух режимов и перенос финальных параметров из `blockcheck2.sh` вынесены в отдельную инструкцию : [Настройка OpenBSD](openbsd.md).
